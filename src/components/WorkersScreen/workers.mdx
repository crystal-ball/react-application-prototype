...
We have a "working" pool of workers that we can queue async work with 🎉
...
```javascript
// export another configuraiton entry creating a webworker bundle
{
  name: 'idWorker',
  entry: './src/components/WorkersScreen/random-id.js',
  output: {
...
const blob = new Blob([`importScripts('http://localhost:3000/id-worker.js');`], {
